...
function cookiesDirective(disclosurePos, displayTimes, privacyPolicyUri, cookieScripts,policyDescription,policyAcceptUserText,policyButtonText) {
	
	
	//description
	var getPolicyDescription = policyDescription;
					...
					if(displayTimes > 0) {
						...
						if(displayTimes >= cdReadCookie('cookiesDisclosureCount')) {
							...
							if(cookieScripts) {
								...
								cdHandler(disclosurePosition,privacyPolicyUri,cookieScripts,getPolicyDescription,policyAcceptUserText,policyButtonText,displayTimes);

							} else {
						
								cdHandler(disclosurePosition,privacyPolicyUri,getPolicyDescription,policyAcceptUserText,policyButtonText,displayTimes);
		...
		if(!cdReadCookie('cookiesDirective')) {
			...
			if(displayTimes > 0) {
				...
				if(cookieScripts) {
					...
					cdHandler(disclosurePosition,privacyPolicyUri,cookieScripts,policyAcceptUserText,policyButtonText,displayTimes);
...
function cdHandler(disclosurePosition, privacyPolicyUri, cookieScripts,getPolicyDescription,policyAcceptUserText,policyButtonText,displayTimes) {
	
	// Our main disclosure script
		
	var displaySeconds = displayTimes; // Alter this to remove the banner after number of seconds
	...
	var setPolicyDescription = getPolicyDescription;
	...
	if(setPolicyDescription != '')
	{
		disclosure += setPolicyDescription;
		
		if(policyAcceptUserText != ''){
		
			disclosure += policyAcceptUserText +'&nbsp;<input type="checkbox" name="epdagree" id="epdagree" />&nbsp;';
		}
		else
		{
...
var isFroyoOrOlder = isAndroid && !isGingerbread && !isHoneycombOrNewer;
		...
		if(policyButtonText != '')
		{
			...
			if(isGingerbread)
			{
				disclosure += '<input type="submit" class="androidButton" name="epdsubmit" id="epdsubmit" value="'+policyButtonText+'"/><br/><br/></div></div>';
			}
			else{
			
			
	    	    disclosure += '<input type="submit" name="epdsubmit" id="epdsubmit" value="'+policyButtonText+'"/><br/><br/></div></div>';
			}
	...
	else
	{
		...
		disclosure += "please add content in Cookies Policy Settings, Thank you."
